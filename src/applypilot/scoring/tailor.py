...
def _safe_write(path: Path, content: str, retries: int = 3) -> None:
    """Write via temp file + atomic rename, with retries for Windows locks."""
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                log.warning("Retrying write to %s (attempt %d)", path, attempt + 2)
            else:
                raise
...
def _refine_resume_json(data: dict, job_text: str) -> dict:
    """Second pass: refine tailored resume JSON for natural voice and flow.

    Takes the structured JSON from the first tailoring pass and polishes
    bullet points and summary for more natural, human-sounding language
    while preserving all facts, metrics, and structure.
    """
    client = _get_tailor_client()

    prompt = """You are refining a tailored resume. The JSON below was generated by a resume tailoring engine.
Your ONLY job is to improve how it reads. Make it sound like a real person wrote it, not a machine.

RULES:
- Make every bullet sound natural and direct
- Vary sentence structure across bullets - avoid repetitive verb-first patterns
- Remove any remaining stiffness or template-sounding phrasing
- Keep ALL facts, metrics, numbers, company names, and technical details EXACTLY as-is
- Keep the JSON structure IDENTICAL - same keys, same number of entries, same number of bullets
- Do not add or remove any information
- Do not change any numbers or metrics
- Do not add new skills, tools, or technologies
- No em dashes. Use commas, periods, or hyphens.

Return ONLY the refined JSON object. No markdown fences. No commentary."""

    messages = [
        {"role": "system", "content": prompt},
        {"role": "user", "content": (
            f"JOB CONTEXT:\n{job_text[:2000]}\n\n"
            f"RESUME JSON TO REFINE:\n{json.dumps(data, indent=2)}\n\n"
            "Return the refined JSON:"
        )},
    ]

    raw = client.chat(messages, max_tokens=8192, temperature=0.55)
    try:
        refined = extract_json(raw)
        # Sanity check: ensure required fields survived
        for key in ("title", "summary", "skills", "experience"):
            if key not in refined:
                log.warning("Refinement dropped '%s' field, using original", key)
                return data
        return refined
    except ValueError:
        log.warning("Refinement pass failed to return valid JSON, using original")
        return data
...
def tailor_resume(
    ...
    for attempt in range(max_retries + 1):
        ...
        # Two-pass refinement: polish JSON for natural voice before assembly
        data = _refine_resume_json(data, job_text)
...
def run_tailoring(min_score: int = 7, limit: int = 0,
    ...
    for job in jobs:
        ...
        try:
            ...
            _safe_write(job_path, job_desc)

            # Save validation report
            report_path = TAILORED_DIR / f"{prefix}_REPORT.json"
            _safe_write(report_path, json.dumps(report, indent=2))
        ...
        # Persist to DB immediately so progress survives crashes/interrupts
